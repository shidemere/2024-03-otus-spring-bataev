...
class BookControllerTest {
    ...
    @Test
    @DisplayName("Проверка получения ошибки 403 при попытке получить список книг без нужных прав")
    @WithMockUser(
            username = "test",
            authorities = {"ROLE_TEST"}
    )
    void getBooks_noPermission_error403() throws Exception {
        List<BookDto> books = List.of(
                new BookDto(1L, "Book One", new AuthorDto(1L, "Author One"), new GenreDto(1L, "Genre One")),
                new BookDto(2L, "Book Two", new AuthorDto(2L, "Author Two"), new GenreDto(2L, "Genre Two"))
        );
    ...
    @Test
    @DisplayName("Проверка получения ошибки 403 при запросе без нужных прав")
    @WithMockUser(
            username = "test",
            authorities = {"ROLE_TEST"}
    )
    void getBookById_noPermission_error403() throws Exception {
        long bookId = 1L;
        BookDto book = new BookDto(bookId, "Книга", new AuthorDto(1L, "Автор"), new GenreDto(1L, "Жанр"));
    ...
    @Test
    @DisplayName("Проверка получения ошибки 403 при запросе на сохранение без нужных прав")
    @WithMockUser(
            username = "user",
            authorities = {"ROLE_USER"}
    )
    void createBook_noPermission_error403() throws Exception {
        BookCreateDto requestDto = new BookCreateDto("Книга", 1L, 2L);
        BookDto responseDto = new BookDto(1L, "Книга", new AuthorDto(1L, "Автор один"), new GenreDto(2L, "Жанр два"));
    ...
    @Test
    @DisplayName("Проверка на получение ошибки 403 при отсутсивии нужных прав на обновление")
    @WithMockUser(
            username = "user",
            authorities = {"ROLE_USER"}
    )
    void updateBook_noPermission_error403() throws Exception {
        BookUpdateDto requestDto = new BookUpdateDto(1L, "Обновленная книга", 1L, 2L);
        BookDto responseDto = new BookDto(1L, "Обновленная книга", new AuthorDto(1L, "Автор один"), new GenreDto(2L, "Жанр два"));
    ...
    void deleteBook_noPermission_error403() throws Exception {
        Mockito.doNothing().when(bookService).deleteById(1L);
