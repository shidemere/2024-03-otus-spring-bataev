...
class AuthorControllerTest {
    ...
    @Test
    @DisplayName("Проверка получения списка авторов без нужных на то прав")
    @WithMockUser(
            username = "test",
            authorities = {"ROLE_TEST"}
    )
    void getAuthor_noPermission_error403() throws Exception {
        List<AuthorDto> authors = List.of(
                new AuthorDto(1L, "Донателло"),
                new AuthorDto(2L, "Микиланджело"),
                new AuthorDto(3L, "Рафаэль")
    ...
    @Test
    @DisplayName("Проверка получения статуса 403 при попытке добавления автора без нужных на то прав")
    @WithMockUser(
            username = "user",
            authorities = {"ROLE_USER"}
    )
    void addAuthor_noPermission_error403() throws Exception {
        AuthorDto requestDto = new AuthorDto(null, "Леонардо");
        AuthorDto responseDto = new AuthorDto(1L, "Леонардо");
